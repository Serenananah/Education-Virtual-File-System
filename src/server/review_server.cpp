...
ReviewServer::handle_assign_reviewer(const protocol::Message &msg,
  ...
  // Add to assignments.txt (round:reviewer)
  std::string reviewers_file = assignments_file(paper_dir);
  ...
  std::string line = round + ":" + reviewer + "\n";
  vfs_->write(fd, line.data(), line.size());
  vfs_->close(fd);

  // Update State to UNDER_REVIEW and persist
